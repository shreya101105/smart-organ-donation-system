    ...
    <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ position: 'relative', width: size, height: size }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-color)' }}>{percent}%</span>
          {subtitle && <span style={{ fontSize: '0.65rem', color: 'var(--muted-color)', textTransform: 'uppercase' }}>{subtitle}</span>}
        </div>
      </div>
      {title && <h5 style={{ marginTop: '12px', fontSize: '0.9rem', color: 'var(--text-color)', fontWeight: 600 }}>{title}</h5>}
    </div>
  );
};
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', gap: '8px' }}>
        {data.map((item, idx) => (
          <div key={idx} style={{ flex: 1, textCenter: 'center', textAlign: 'center', fontSize: '0.75rem', color: 'var(--muted-color)', fontWeight: '500' }}>
            {item.label}
          </div>
        ))}
      </div>
